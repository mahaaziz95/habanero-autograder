...
app.post('/mark_final/:run_id', function(req, res, next) {
    ...
    function(rows) {
        ...
        function(rows) {
           ...
           'Inserted final run into final_runs table.');
       });
...
function is_final(run_id, user_id, assignment_id, cb, res, req) {
    ...
    pgquery_no_err('SELECT * FROM final_runs WHERE user_id=($1) AND assignment_id=($2) ORDER BY final_run_id DESC LIMIT 1', [user_id, assignment_id], res, req, function(rows) {
      console.log(rows)
      console.log("rows above")
      if (rows.length != 1) {
          return cb(false);
      }
      // Make sure that the run ID is the run ID that they actually want
      if (rows[0].run_id == run_id) {
          return cb(true, rows[0].status);
      }
        ...
        pgquery_no_err("SELECT name FROM assignments WHERE assignment_id=($1)",
            ...
            pgquery_no_err("SELECT * FROM users WHERE user_id=($1)", [user_id],
                ...
                if (!fs.existsSync(run_dir)) {
                  ...
                  // Will return true or false if the run is marked final or not
                  is_final(run_id, user_id, assignment_id, function(marked_final, status) {
